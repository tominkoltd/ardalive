/* --------------------------------------------------------------------------
   ...
   - Applies HTML diffs via idiomorph, hot-swaps CSS via blob URLs
 ...
 */
function init() {
	// Expand SSI placeholders once so morphing works on a stable tree.
	// morphStyle innerHTML: only <body>'s children are morphed, so
	// style/class/id set on the real <body> tag stay intact.
	const firstPass = loadSSI(document.body.innerHTML);
	Idiomorph.morph(document.body, firstPass, { ...morphCfg, morphStyle: 'innerHTML' });
...
// Idiomorph config shared across all morph calls:
// - restoreFocus / ignoreActiveValue: don't fight the user interacting with
//   the preview page (focused form inputs keep their typed value and focus)
// - head.style 'morph': head is diffed like any element so inline <style>
//   edits hot-update and our stylesheet guards below apply there too
// - Skip <img> whose src hasn't changed (prevents re-fetch/flicker)
// - <link rel="stylesheet"> handling: ArdaLive serves unsaved CSS via blob
//   URLs, so a morph must never revert a blob link to its disk href, remove
//   it, or add a duplicate disk-href link next to it
...
const morphCfg = {
	...
	callbacks: {
		beforeNodeMorphed(oldNode, newNode) {
			if (!oldNode.tagName) return true
			if (oldNode.tagName === 'IMG' && newNode.tagName === 'IMG' &&
				oldNode.getAttribute('src') === newNode.getAttribute('src')) return false
			if (isStyleLink(oldNode)) return false
			return true
		},
		beforeNodeRemoved(node) {
			if (isStyleLink(node) && node.href && node.href.startsWith('blob:')) return false
			return true
		},
			...
			if (isStyleLink(node)) {
				try {
					...
					if (u.origin === location.origin) {
						const tracked = LINKS[u.pathname + u.search]
						if (tracked && tracked.element && tracked.element.isConnected) return false
					}
				} catch (e) { /* fall through: allow the add */ }
			}
			return true
		}
	}
...
function onServerMessage(event) {
	...
	if (change.type === 'ME') {
		...
		if (/<head\b/i.test(updatePacket.data)) {
			Idiomorph.morph(document.head, newDoc.head, morphCfg);
		}
		Idiomorph.morph(document.body, newDoc.body, morphCfg);
		return;
	}
...
/* --------------------------------------------------------------------------
   Idiomorph Library (Bundled)
   --------------------------------------------------------------------------
   idiomorph v0.7.4, included below unmodified (minified build), under its
   original Zero-Clause BSD license.
